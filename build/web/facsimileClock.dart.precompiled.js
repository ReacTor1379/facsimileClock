...
["_interceptors", "dart:_interceptors", , J, {
  ...
  Interceptor: {
    ...
    "%": "AnimationPlayerEvent|ApplicationCacheErrorEvent|AudioProcessingEvent|AutocompleteErrorEvent|BeforeUnloadEvent|CSSFontFaceLoadEvent|ClipboardEvent|CloseEvent|CompositionEvent|CustomEvent|DOMError|DeviceLightEvent|DeviceMotionEvent|DeviceOrientationEvent|DragEvent|ErrorEvent|Event|FetchEvent|FileError|FocusEvent|GamepadEvent|HashChangeEvent|IDBVersionChangeEvent|InputEvent|InstallEvent|InstallPhaseEvent|KeyboardEvent|MIDIConnectionEvent|MIDIMessageEvent|MSPointerEvent|MediaError|MediaKeyError|MediaKeyEvent|MediaKeyMessageEvent|MediaKeyNeededEvent|MediaStreamEvent|MediaStreamTrackEvent|MessageEvent|MouseEvent|MouseScrollEvent|MouseWheelEvent|MutationEvent|Navigator|NavigatorCPU|NavigatorUserMediaError|OfflineAudioCompletionEvent|OverflowEvent|PageTransitionEvent|PointerEvent|PopStateEvent|PositionError|ProgressEvent|PushEvent|RTCDTMFToneChangeEvent|RTCDataChannelEvent|RTCIceCandidateEvent|ResourceProgressEvent|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGZoomEvent|SecurityPolicyViolationEvent|SpeechRecognitionError|SpeechRecognitionEvent|SpeechSynthesisEvent|StorageEvent|TextEvent|TouchEvent|TrackEvent|TransitionEvent|UIEvent|WebGLContextEvent|WebKitAnimationEvent|WebKitTransitionEvent|WheelEvent|XMLHttpRequestProgressEvent"
  },
  JSBool: {
    "^": "Interceptor;",
    toString$0: function(receiver) {
...
["clock", "facsimileClock.dart", , Q, {
  "^": "",
  main: [function() {
    Q.Panel$();
  }, "call$0", "main$closure", 0, 0, 0],
  Ball: {
    "^": "Object;ballDiv,color",
    createBall$1: function(color) {
      var ballDiv = document.createElement("div", null);
      J.set$backgroundColor$x(ballDiv.style, color);
      J.set$width$x(ballDiv.style, "16px");
      J.set$height$x(ballDiv.style, "16px");
      J.set$borderRadius$x(ballDiv.style, "8px");
      this.ballDiv = ballDiv;
    },
    changeColor$1: function(color) {
      var t1, t2, t3;
      t1 = $.get$Ball_BALL_COLOR();
      t2 = t1.containsKey$1(color);
      t3 = this.ballDiv;
      if (t2)
        J.set$backgroundColor$x(t3.style, t1.$index(0, color));
      else
        J.set$backgroundColor$x(t3.style, t1.$index(0, "GREY"));
      this.color = color;
    },
    Ball$1: function(color) {
      var t1 = $.get$Ball_BALL_COLOR();
      if (t1.containsKey$1(color))
        this.createBall$1(t1.$index(0, color));
      else
        this.createBall$1(t1.$index(0, "GREY"));
      this.color = color;
    },
    static: {"^": "Ball_RADIUS,Ball_BALL_COLOR", Ball$: function(color) {
        var t1 = new Q.Ball(null, null);
        t1.Ball$1(color);
        return t1;
      }}
  },
  MoveBall: {
    "^": "Ball;x>,y>,vx<,vy,ax,ay,lastTime,delta,ballDiv,color",
    isCollide$0: function() {
      J.forEach$1$ax($.get$CollideBoard_collideBalls(), new Q.MoveBall_isCollide_closure(this));
    },
    isApproaching$2: function(otherBall, dis) {
      var t1, t2, t3, t4, t5, t6, t7, t8, t9, nx, ny;
      t1 = J.get$x$x(otherBall);
      t2 = otherBall.get$vx();
      if (typeof t2 !== "number")
        return t2.$mul();
      t3 = this.delta;
      ...
      if (typeof t8 !== "number")
        return t8.$add();
      nx = Math.abs(t6 + t7 / t3 - (t1 + t2 / t3));
      ny = Math.abs(t8 + t9 / t3 - (t4 + t5 / t3));
      if (Math.sqrt(H.checkNum(nx * nx + ny * ny)) < dis)
        return true;
      return false;
    },
    MoveBall$1: function(color) {
      var t1;
      this.createBall$1($.get$MoveBall_BALL_COLOR().$index(0, color));
      this.color = color;
      t1 = Date.now();
      ...
      this.ax = 0;
      this.ay = 400;
      this.vx = Q.MoveBall_randomVelocity();
      this.vy = Q.MoveBall_randomVelocity();
    },
    static: {"^": "MoveBall_BALL_COLOR,MoveBall_GRAVITY,MoveBall_RESTITUTION,MoveBall_MIN_VELOCITY,MoveBall_INIT_VELOCITY,MoveBall_random", MoveBall_randomVelocity: function() {
        var t1 = $.MoveBall_random;
        if (t1 == null) {
          $.MoveBall_random = C.C__JSRandom;
          t1 = C.C__JSRandom;
        }
        return (t1.nextDouble$0() - 0.5) * 800;
      }}
  },
  MoveBall_isCollide_closure: {
    "^": "Closure:9;this_0",
    call$1: function(otherBall) {
      var t1, t2, t3, t4, dx, dy, dis, t5, impactSpeed;
      t1 = this.this_0;
      t2 = J.getInterceptor(otherBall);
      if (!t2.$eq(otherBall, t1) && t2.get$x(otherBall) != null) {
        t3 = t1.x;
        t4 = t2.get$x(otherBall);
        if (typeof t3 !== "number")
          return t3.$sub();
        if (typeof t4 !== "number")
          return H.iae(t4);
        dx = Math.abs(t3 - t4);
        t4 = t1.y;
        t2 = t2.get$y(otherBall);
        if (typeof t4 !== "number")
          return t4.$sub();
        if (typeof t2 !== "number")
          return H.iae(t2);
        dy = Math.abs(t4 - t2);
        dis = Math.sqrt(H.checkNum(dx * dx + dy * dy));
        if (dis < 16)
          if (t1.isApproaching$2(otherBall, dis)) {
            if (dis === 0)
              return;
            dx /= dis;
            dy /= dis;
  ...
  NumberBoard: {
    "^": "Object;balls,pixels,color,root",
    setPixels$1: function(num) {
      var y, x, t1, t2, t3, t4, moveBall, pos;
      for (y = 0; y < 7; ++y)
        for (x = 0; x < 4; ++x) {
          t1 = this.pixels[y];
          t2 = t1.length;
          ...
          if (t3 == null) {
            if (num < 0 || num >= 10)
              return H.ioore(C.List_e3I, num);
            t2 = C.List_e3I[num][y][x];
            t1[x] = t2;
            if (t2 === 1) {
              t1 = this.balls[y];
              if (x >= t1.length)
                return H.ioore(t1, x);
              t1[x].changeColor$1(this.color);
            }
          } else {
            if (num < 0 || num >= 10)
              return H.ioore(C.List_e3I, num);
            t4 = C.List_e3I[num][y][x];
            if (t3 !== t4) {
              t1[x] = t4;
              if (x >= t2)
                return H.ioore(t1, x);
              ...
              if (t4 === 1) {
                ...
                t1 = t1[x];
                t2 = t1.color;
                moveBall = new Q.MoveBall(null, null, null, null, null, null, null, null, null, null);
                t3 = $.get$Ball_BALL_COLOR();
                if (t3.containsKey$1("GREY"))
                  moveBall.createBall$1(t3.$index(0, "GREY"));
                else
                  moveBall.createBall$1(t3.$index(0, "GREY"));
                moveBall.color = "GREY";
                moveBall.MoveBall$1(t2);
                pos = t1.ballDiv.getBoundingClientRect();
                t1 = $.get$CollideBoard_root();
                t2 = J.getInterceptor$x(pos);
                t3 = t2.get$left(pos);
                t2 = t2.get$top(pos);
                t1.appendChild(moveBall.ballDiv);
                J.set$position$x(moveBall.ballDiv.style, "absolute");
                J.set$transform$x(moveBall.ballDiv.style, "translate(" + H.S(t3) + "px, " + H.S(t2) + "px)");
                J.set$position$x(moveBall.ballDiv.style, "absolute");
                $.get$CollideBoard_collideBalls().push(moveBall);
                t2 = this.balls[y];
                if (x >= t2.length)
                  return H.ioore(t2, x);
                t2[x].changeColor$1("GREY");
              }
            }
          }
        }
    },
    NumberBoard$1: function(color) {
      var i, t1, t2, y, x, t3, t4;
      this.color = color;
      this.root = document.createElement("div", null);
      this.balls = H.setRuntimeTypeInfo(Array(7), [[P.List, Q.Ball]]);
      this.pixels = H.setRuntimeTypeInfo(Array(7), [[P.List, P.$int]]);
      for (i = 0; i < 7; ++i) {
        t1 = this.balls;
        t2 = [];
        t2.$builtinTypeInfo = [Q.Ball];
        t1[i] = t2;
        J.set$length$asx(this.balls[i], 4);
        t1 = this.pixels;
        t2 = [];
        t2.$builtinTypeInfo = [P.$int];
        t1[i] = t2;
        J.set$length$asx(this.pixels[i], 4);
      }
      for (y = 0; y < 7; ++y)
        for (t1 = y * 21, x = 0; x < 4; ++x) {
          t2 = this.balls[y];
          t3 = new Q.Ball(null, null);
          t4 = $.get$Ball_BALL_COLOR();
          if (t4.containsKey$1("GREY"))
            t3.createBall$1(t4.$index(0, "GREY"));
          else
            t3.createBall$1(t4.$index(0, "GREY"));
          t3.color = "GREY";
          if (x >= t2.length)
          ...
          t3 = this.root;
          t3.toString;
          t3.appendChild(t2.ballDiv);
          J.set$position$x(t2.ballDiv.style, "absolute");
          J.set$transform$x(t2.ballDiv.style, "translate(" + x * 21 + "px, " + t1 + "px)");
        }
    },
    static: {"^": "NumberBoard_WIDTH_BALL_NUM,NumberBoard_HEIGHT_BALL_NUM,NumberBoard_PADDING_SPACE,NumberBoard_WIDTH,NumberBoard_HEIGHT", NumberBoard$: function(color) {
        var t1 = new Q.NumberBoard(null, null, null, null);
        t1.NumberBoard$1(color);
        return t1;
      }}
  },
  ColonBoard: {
    "^": "Object;root",
    ColonBoard$0: function() {
      var t1, ball;
      t1 = document.createElement("div", null);
      this.root = t1;
      J.set$position$x(t1.style, "absolute");
      ball = Q.Ball$("GREY");
      this.root.appendChild(ball.ballDiv);
      J.set$transform$x(ball.ballDiv.style, "translate(0px, 39px)");
      ball = Q.Ball$("GREY");
      this.root.appendChild(ball.ballDiv);
      J.set$transform$x(ball.ballDiv.style, "translate(0px, 65px)");
    },
    static: {"^": "ColonBoard_PADDING_SPACE,ColonBoard_WIDTH"}
  },
  CollideBoard: {
    "^": "Object;",
    collide$1: [function(time) {
      var t1, i, t2, t3, t4, t5, t6;
      for (t1 = J.getInterceptor$n(time), i = 0; t2 = $.get$CollideBoard_collideBalls(), i < t2.length; ++i) {
        t2 = t2[i];
        t3 = t1.$sub(time, t2.lastTime);
        if (typeof t3 !== "number")
          return H.iae(t3);
        t2.delta = P.max(1000 / t3, 30);
        t2.lastTime = time;
        if (t2.x == null) {
          t2.x = J.get$left$x(t2.ballDiv.getBoundingClientRect());
          t2.y = J.get$top$x(t2.ballDiv.getBoundingClientRect());
        } else
          t2.isCollide$0();
        t3 = t2.x;
        t4 = t2.vx;
        ...
        if (!(t4 <= -16)) {
          ...
          t3 = true;
        if (t3) {
          J.remove$1$ax($.get$CollideBoard_collideBalls(), t2);
          t3 = t2.ballDiv;
          t4 = t3.parentElement;
          t4.toString;
          new W._ChildNodeListLazy(t4).remove$1(0, t3);
        }
        t3 = t2.y;
        ...
        t4 = t2.ballDiv;
        t2 = t2.x;
        J.set$transform$x(t4.style, "translate(" + H.S(t2) + "px, " + H.S(t3) + "px)");
      }
      t1 = window;
      t2 = this.get$collide();
      C.Window_methods._ensureRequestAnimationFrame$0(t1);
      C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t2));
    }, "call$1", "get$collide", 2, 0, 12],
    static: {"^": "CollideBoard_collideBalls,CollideBoard_root"}
  },
  Panel: {
    "^": "Object;hours,minutes,seconds,displayedHour,displayedMinute,displayedSecond,container,collideBoard",
    initBoard$4: function(boards, x, y, color) {
      var i, t1;
      for (i = 0; i < 2; ++i) {
        t1 = Q.NumberBoard$(color);
        boards[i] = t1;
        this.container.appendChild(t1.root);
        J.set$transform$x(boards[i].root.style, "translate(" + H.S(x) + "px, " + H.S(y) + "px)");
        x += 99;
      }
      return x;
    },
    initColon$2: function(x, y) {
      var colon = new Q.ColonBoard(null);
      colon.ColonBoard$0();
      this.container.appendChild(colon.root);
      J.set$transform$x(colon.root.style, "translate(" + H.S(x) + "px, " + H.S(y) + "px)");
      return x + 36;
    },
    updateTime$1: function(now) {
      if (H.Primitives_getSeconds(now) !== this.displayedSecond) {
        ...
        if (H.Primitives_getHours(now) !== this.displayedHour || H.Primitives_getHours(now) === 0) {
          this.setDigits$2(this.pad2$1(H.Primitives_getHours(now)), this.hours);
          this.displayedHour = H.Primitives_getHours(now);
        }
        if (H.Primitives_getMinutes(now) !== this.displayedMinute || H.Primitives_getMinutes(now) === 0) {
          this.setDigits$2(this.pad2$1(H.Primitives_getMinutes(now)), this.minutes);
          this.displayedMinute = H.Primitives_getMinutes(now);
        }
      }
    },
    setDigits$2: function(digits, timeBoard) {
      var t1, i, t2, t3;
      for (t1 = digits.length, i = 0; i < t1; ++i) {
        t2 = C.JSString_methods.codeUnitAt$1(digits, i);
        t3 = C.JSString_methods.codeUnitAt$1("0", 0);
        if (i >= 2)
          return H.ioore(timeBoard, i);
        timeBoard[i].setPixels$1(t2 - t3);
      }
    },
    pad2$1: function(num) {
    ...
    Panel$0: function() {
      var t1, t2, curY, t3;
      t1 = document.querySelector("#container");
      this.container = t1;
      t1.toString;
      t2 = C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(t1.clientWidth));
      curY = (C.JSNumber_methods.toInt$0(C.JSNumber_methods.roundToDouble$0(t1.clientHeight)) - 142) / 3;
      this.initBoard$4(this.seconds, this.initColon$2(this.initBoard$4(this.minutes, this.initColon$2(this.initBoard$4(this.hours, (t2 - 646) / 2, curY, "BLUE"), curY), curY, "RED"), curY), curY, "GREEN");
      t2 = new P.DateTime(Date.now(), false);
      t2.DateTime$_now$0();
      this.updateTime$1(t2);
      t2 = new Q.CollideBoard();
      J.set$transform$x($.get$CollideBoard_root().style, "translate(0px, 0px)");
      t1 = window;
      t3 = t2.get$collide();
      C.Window_methods._ensureRequestAnimationFrame$0(t1);
      C.Window_methods._requestAnimationFrame$1(t1, W._wrapZone(t3));
      this.collideBoard = t2;
      P.Timer_Timer$periodic(C.Duration_1000000, new Q.Panel_closure(this));
    },
    static: {"^": "Panel_PADDING_SPACE", Panel$: function() {
        var t1 = new Q.Panel(H.setRuntimeTypeInfo(Array(2), [Q.NumberBoard]), H.setRuntimeTypeInfo(Array(2), [Q.NumberBoard]), H.setRuntimeTypeInfo(Array(2), [Q.NumberBoard]), 0, 0, 0, null, null);
        t1.Panel$0();
        return t1;
      }}
  },
...
;(function() {
  ...
  _.$isDuration = TRUE;
  _.$isObject = TRUE;
  Q.NumberBoard.$isObject = TRUE;
  Q.MoveBall.$isObject = TRUE;
...
J.set$borderRadius$x = function(receiver, value) {
  return J.getInterceptor$x(receiver).set$borderRadius(receiver, value);
};
...
Isolate.$lazy($, "BALL_COLOR", "Ball_BALL_COLOR", "get$Ball_BALL_COLOR", function() {
  return P.LinkedHashMap_LinkedHashMap$_literal(["GREY", "#c9c9c9", "BLUE", "#13acfa", "RED", "#c0000b", "GREEN", "#009a49", "BLACK", "#000000"], null, null);
});
Isolate.$lazy($, "BALL_COLOR", "MoveBall_BALL_COLOR", "get$MoveBall_BALL_COLOR", function() {
  return P.LinkedHashMap_LinkedHashMap$_literal(["GREY", "#c9c9c9", "BLUE", "#13acfa", "RED", "#c0000b", "GREEN", "#009a49", "BLACK", "#000000"], null, null);
});
Isolate.$lazy($, "collideBalls", "CollideBoard_collideBalls", "get$CollideBoard_collideBalls", function() {
  return H.setRuntimeTypeInfo([], [Q.MoveBall]);
});
Isolate.$lazy($, "root", "CollideBoard_root", "get$CollideBoard_root", function() {
  return document.querySelector("#collideArea");
});
...
//# sourceMappingURL=facsimileClock.dart.js.map
{
  var message =
...
function dart_precompiled($collectedClasses) {
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Ball.prototype = $desc;
  function MoveBall(x, y, vx, vy, ax, ay, lastTime, delta, ballDiv, color) {
    this.x = x;
    this.y = y;
    this.vx = vx;
    this.vy = vy;
  ...
  MoveBall.builtin$cls = "MoveBall";
  if (!"name" in MoveBall)
    MoveBall.name = "MoveBall";
  $desc = $collectedClasses.MoveBall;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MoveBall.prototype = $desc;
  MoveBall.prototype.get$x = function(receiver) {
    return this.x;
  };
  MoveBall.prototype.get$y = function(receiver) {
    return this.y;
  };
  MoveBall.prototype.get$vx = function() {
    return this.vx;
  };
  function MoveBall_isCollide_closure(this_0) {
    this.this_0 = this_0;
  }
  MoveBall_isCollide_closure.builtin$cls = "MoveBall_isCollide_closure";
  if (!"name" in MoveBall_isCollide_closure)
    MoveBall_isCollide_closure.name = "MoveBall_isCollide_closure";
  $desc = $collectedClasses.MoveBall_isCollide_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  MoveBall_isCollide_closure.prototype = $desc;
  function NumberBoard(balls, pixels, color, root) {
    this.balls = balls;
    this.pixels = pixels;
    this.color = color;
    this.root = root;
  }
  NumberBoard.builtin$cls = "NumberBoard";
  if (!"name" in NumberBoard)
    NumberBoard.name = "NumberBoard";
  $desc = $collectedClasses.NumberBoard;
  if ($desc instanceof Array)
    $desc = $desc[1];
  NumberBoard.prototype = $desc;
  function ColonBoard(root) {
    this.root = root;
  }
  ColonBoard.builtin$cls = "ColonBoard";
  if (!"name" in ColonBoard)
    ColonBoard.name = "ColonBoard";
  $desc = $collectedClasses.ColonBoard;
  if ($desc instanceof Array)
    $desc = $desc[1];
  ColonBoard.prototype = $desc;
  function CollideBoard() {
  }
  CollideBoard.builtin$cls = "CollideBoard";
  if (!"name" in CollideBoard)
    CollideBoard.name = "CollideBoard";
  $desc = $collectedClasses.CollideBoard;
  if ($desc instanceof Array)
    $desc = $desc[1];
  CollideBoard.prototype = $desc;
  function Panel(hours, minutes, seconds, displayedHour, displayedMinute, displayedSecond, container, collideBoard) {
    this.hours = hours;
    ...
    this.collideBoard = collideBoard;
  }
  Panel.builtin$cls = "Panel";
  if (!"name" in Panel)
    Panel.name = "Panel";
  $desc = $collectedClasses.Panel;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Panel.prototype = $desc;
  function Panel_closure(this_0) {
    this.this_0 = this_0;
  }
  Panel_closure.builtin$cls = "Panel_closure";
  if (!"name" in Panel_closure)
    Panel_closure.name = "Panel_closure";
  $desc = $collectedClasses.Panel_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Panel_closure.prototype = $desc;
  ...
  function AsyncError(stackTrace) {
    this.stackTrace = stackTrace;
  }
